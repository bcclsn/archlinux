...
if (ping -q -c 1 -W 1 8.8.8.8 >/dev/null || ping -q -c 1 -W 1 google.com >/dev/null) ; then
   ...
   duplicity --full-if-older-than 1M --name=$NAME --log-file $LOG $DIR mega://$USER:$PASS@$HOST/$MDIR

   # deleting full backups older than 2 months (2) #
   duplicity remove-all-but-n-full 2 --log-file $LOG --force mega://$USER:$PASS@$HOST/$MDIR

   # to restore a folder from your backup #
   # comment the previuos command and set the directory
   #RESTORE_DIR=
   #duplicity restore --log-file $LOG mega://$USER:$PASS@$HOST/$MDIR $RESTORE_DIR
